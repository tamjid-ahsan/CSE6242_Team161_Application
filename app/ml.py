...
def predict_from_user_preference(preferences, 
    ...
    preference_mappings = {'p_temp': 'avg_temp', 'p_rain': 'avg_rain', 'p_snow': 'avg_snow',
                           'p_education': 'num_postsecondary_institutions', 'p_health': 'health_rating',
                           'p_income': 'avg_salary_per_earner', 'p_housing': 'housing_cost', 'p_politics': 'dem_lead',
                           'p_density': 'density'}
    ...
    for value, method in value_mappings.items():
        ...
        matching_inputs = [input_name for input_name, input_value in selected_preferences.items() if
                           input_value == value]
    ...
    first_prediction['climate'] = climate_vars.drop(columns=climate_vars.columns[(climate_vars == 0).all()]).mean(
        axis=1)
    ...
    scores['final_score'] = scores[
        ['climate_score', 'cost_score', 'education_score', 'health_score', 'politics_score', 'density_score']].sum(
        axis=1)
    ...
    second_prediction['climate'] = climate_vars.drop(columns=climate_vars.columns[(climate_vars == 0).all()]).mean(
        axis=1)
    ...
    recommendation_data = best_cluster_data[
        ['lat', 'lng', 'city', 'state_name', 'population', 'density', 'avg_temp', 'avg_snow',

...
def predict_from_user_preference(
    preferences,
    weights,
    ...
    prediction_data[raw_predictors] = scaler.fit_transform(
        prediction_data[raw_predictors]
    )
    prediction_data[raw_predictors] = prediction_data[raw_predictors].apply(
        custom_rescale
    )

    # combine housing into one column based on average of rent + own
    prediction_data["housing_cost"] = prediction_data[
        ["recent_rental_price", "recent_home_value"]
    ].mean(axis=1)
    prediction_data = prediction_data.drop(
        ["recent_rental_price", "recent_home_value"], axis=1
    )
    ...
    preference_mappings = {
        ...
        "p_rain": "avg_rain",
        "p_snow": "avg_snow",
        "p_education": "num_postsecondary_institutions",
        "p_health": "health_rating",
        "p_income": "avg_salary_per_earner",
        "p_housing": "housing_cost",
        "p_politics": "dem_lead",
        "p_density": "density",
    }
    ...
    for value, method in value_mappings.items():
        ...
        matching_inputs = [
            input_name
            for input_name, input_value in selected_preferences.items()
    ...
    # start with climate + cost variables since there are multiple feature
    # components and average calculations will change if user doesn't have preference
    climate_vars = first_prediction[["avg_temp", "avg_snow", "avg_rain"]]
    first_prediction["climate"] = climate_vars.drop(
        columns=climate_vars.columns[(climate_vars == 0).all()]
    ).mean(axis=1)

    cost_vars = first_prediction[["avg_salary_per_earner", "housing_cost"]]
    first_prediction["cost"] = cost_vars.drop(
        columns=cost_vars.columns[(cost_vars == 0).all()]
    ).mean(axis=1)
    ...
    weight_mappings = {
        "w_climate": "climate",
        "w_cost": "cost",
        "w_education": "num_postsecondary_institutions",
        "w_health": "health_rating",
        "w_politics": "dem_lead",
        "w_density": "density",
    }
    ...
    for weight, column in weight_mappings.items():
        scores[weight.replace("w_", "") + "_score"] = (
            first_prediction[column] * selected_weights[weight]
        )
    ...
    best_cluster = scores.sort_values(by="final_score", ascending=False).reset_index()[
        "cluster_label"
    ][0]

    # now that user has been matched with a cluster,
    # look within the cluster to determine which zip codes/cities/states/etc. are best match
    ...
    second_prediction[raw_predictors] = scaler.fit_transform(
        second_prediction[raw_predictors]
    )
    second_prediction[raw_predictors] = second_prediction[raw_predictors].apply(
        custom_rescale
    )

    # combine housing into one column based on average of rent + own
    second_prediction["housing_cost"] = second_prediction[
        ["recent_rental_price", "recent_home_value"]
    ].mean(axis=1)
    second_prediction = second_prediction.drop(
        ["recent_rental_price", "recent_home_value"], axis=1
    )
    ...
    second_prediction["climate"] = climate_vars.drop(
        columns=climate_vars.columns[(climate_vars == 0).all()]
    ).mean(axis=1)

    cost_vars = second_prediction[["avg_salary_per_earner", "housing_cost"]]
    second_prediction["cost"] = cost_vars.drop(
        columns=cost_vars.columns[(cost_vars == 0).all()]
    ).mean(axis=1)
    ...
    for weight, column in weight_mappings.items():
        scores[weight.replace("w_", "") + "_score"] = (
            second_prediction[column] * selected_weights[weight]
        )
    ...
    best_zips = (
        scores.sort_values(by="final_score", ascending=False)
        .reset_index()
        .copy()[["zip", "final_score"]]
    )
    ...
    recommendation_data = (
        recommendation_data.merge(best_zips, on="zip")
        .sort_values(by="rank")
